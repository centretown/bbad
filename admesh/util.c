...
void stl_transform(stl_file *stl, double const *trafo3x4) {
  ...
  for (i_face = 0; i_face < stl->stats.number_of_facets; ++i_face) {
    ...
    for (i_vertex = 0; i_vertex < 3; ++i_vertex) {
      ...
      v_dst->x = (float)(trafo3x4[0] * v_src_x + trafo3x4[1] * v_src_y +
                         trafo3x4[2] * v_src_z + trafo3x4[3]);
      v_dst->y = (float)(trafo3x4[4] * v_src_x + trafo3x4[5] * v_src_y +
                         trafo3x4[6] * v_src_z + trafo3x4[7]);
      v_dst->z = (float)(trafo3x4[8] * v_src_x + trafo3x4[9] * v_src_y +
                         trafo3x4[10] * v_src_z + trafo3x4[11]);
    }
  }
  double det = trafo3x4[0] * trafo3x4[5] * trafo3x4[10] +
               trafo3x4[4] * trafo3x4[9] * trafo3x4[2] +
               trafo3x4[8] * trafo3x4[1] * trafo3x4[6] -
               trafo3x4[0] * trafo3x4[9] * trafo3x4[6] -
               trafo3x4[4] * trafo3x4[1] * trafo3x4[10] -
               trafo3x4[8] * trafo3x4[5] * trafo3x4[2];
  if (det < 0)
...
void stl_get_transform(stl_file const *stl_src, stl_file *stl_dst,
                       double const *trafo3x4) {
  int i_face, i_vertex;
...
extern void stl_get_size(stl_file *stl) {
  ...
  for (i = 0; i < stl->stats.number_of_facets; i++) {
    for (j = 0; j < 3; j++) {
      stl->stats.min.x =
          STL_MIN(stl->stats.min.x, stl->facet_start[i].vertex[j].x);
      stl->stats.min.y =
          STL_MIN(stl->stats.min.y, stl->facet_start[i].vertex[j].y);
      stl->stats.min.z =
          STL_MIN(stl->stats.min.z, stl->facet_start[i].vertex[j].z);
      stl->stats.max.x =
          STL_MAX(stl->stats.max.x, stl->facet_start[i].vertex[j].x);
      stl->stats.max.y =
          STL_MAX(stl->stats.max.y, stl->facet_start[i].vertex[j].y);
      stl->stats.max.z =
          STL_MAX(stl->stats.max.z, stl->facet_start[i].vertex[j].z);
    }
  }
...
void stl_mirror_xy(stl_file *stl) {
  ...
  stl->stats.facets_reversed -=
      stl->stats.number_of_facets; /* for not altering stats */
}
...
static float get_area(stl_facet *facet) {
  ...
  for (i = 0; i < 3; i++) {
    cross[i][0] =
        (((double)facet->vertex[i].y * (double)facet->vertex[(i + 1) % 3].z) -
         ((double)facet->vertex[i].z * (double)facet->vertex[(i + 1) % 3].y));
    cross[i][1] =
        (((double)facet->vertex[i].z * (double)facet->vertex[(i + 1) % 3].x) -
         ((double)facet->vertex[i].x * (double)facet->vertex[(i + 1) % 3].z));
    cross[i][2] =
        (((double)facet->vertex[i].x * (double)facet->vertex[(i + 1) % 3].y) -
...
void stl_repair(stl_file *stl, int fixall_flag, int exact_flag,
                int tolerance_flag, float tolerance, int increment_flag,
                float increment, int nearby_flag, int iterations,
                int remove_unconnected_flag, int fill_holes_flag,
                int normal_directions_flag, int normal_values_flag,
