...
void stl_open(stl_file *stl, const ADMESH_CHAR *file) {
  stl_initialize(stl);
...
void stl_count_facets(stl_file *stl, const ADMESH_CHAR *file) {
  long file_size;
  ...
  if (stl->stats.type == binary) {
    ...
    if ((!fread(&header_num_facets, sizeof(uint32_t), 1, stl->fp)) ||
        (uint32_t)num_facets != le32toh(header_num_facets)) {
      fprintf(
          stderr,
...
void stl_allocate(stl_file *stl) {
  ...
  stl->facet_start =
      (stl_facet *)calloc(stl->stats.number_of_facets, sizeof(stl_facet));
  if (stl->facet_start == NULL)
    perror("stl_initialize");
  stl->stats.facets_malloced = stl->stats.number_of_facets;

  /* Allocate memory for the neighbors list */
  stl->neighbors_start = (stl_neighbors *)calloc(stl->stats.number_of_facets,
                                                 sizeof(stl_neighbors));
  if (stl->facet_start == NULL)
    perror("stl_initialize");
}

void stl_open_merge(stl_file *stl, ADMESH_CHAR *file_to_merge) {
  ...
  /* Record how many facets we have so far from the first file.  We will start
     putting facets in the next position.  Since we're 0-indexed, it'l be the
     same position. */
  ...
  /* Copy what we need to into stl so that we can read the file_to_merge
     directly into it using stl_read:  Save the rest of the valuable info: */
  stl->stats.type = stl_to_merge.stats.type;
  stl->fp = stl_to_merge.fp;

  /* Add the number of facets we already have in stl with what we we found in
     stl_to_merge but haven't read yet. */
  stl->stats.number_of_facets =
      num_facets_so_far + stl_to_merge.stats.number_of_facets;

  /* Allocate enough room for stl->stats.number_of_facets facets and neighbors:
   */
  stl_reallocate(stl);

  /* Read the file to merge directly into stl, adding it to what we have
     already. Start at num_facets_so_far, the index to the first unused facet.
     Also say that this isn't our first time so we should augment stats like min
     and max instead of erasing them. */
  stl_read(stl, num_facets_so_far, 0);

  /* Restore the stl information we overwrote (for stl_read) so that it still
     accurately reflects the subject part: */
...
extern void stl_reallocate(stl_file *stl) {
  ...
  stl->facet_start = (stl_facet *)realloc(
      stl->facet_start, stl->stats.number_of_facets * sizeof(stl_facet));
  if (stl->facet_start == NULL)
    perror("stl_initialize");
  stl->stats.facets_malloced = stl->stats.number_of_facets;
...
void stl_read(stl_file *stl, int first_facet, int first) {
...
#if 1
    {
      // Positive and negative zeros are possible in the floats, which are
      // considered equal by the FP unit. When using a memcmp on raw floats,
      // those numbers report to be different. Unify all +0 and -0 to +0 to make
      // the floats equal under memcmp.
...
#else
    {
      // Due to the nature of the floating point numbers, close to zero values
      // may be represented with singificantly higher precision than the rest of
      // the vertices. Round them to zero.
      float *f = (float *)&facet;
      for (int j = 0; j < 12; ++j, ++f) // 3x vertex + normal: 4x3 = 12 floats
  ...
  stl->stats.bounding_diameter = sqrt(stl->stats.size.x * stl->stats.size.x +
