...
void stl_print_edges(stl_file *stl, FILE *file) {
  ...
  for (i = 0; i < edges_allocated; i++) {
    fprintf(file, "%d, %f, %f, %f, %f, %f, %f\n",
            stl->edge_start[i].facet_number, stl->edge_start[i].p1.x,
            stl->edge_start[i].p1.y, stl->edge_start[i].p1.z,
            stl->edge_start[i].p2.x, stl->edge_start[i].p2.y,
            stl->edge_start[i].p2.z);
  }
}
...
void stl_write_ascii(stl_file *stl, const ADMESH_CHAR *file,
                     const char *label) {
  int i;
  FILE *fp;
  // char      *error_msg;
...
void stl_print_neighbors(stl_file *stl, ADMESH_CHAR *file) {
  int i;
  FILE *fp;
  // char *error_msg;
...
void stl_write_binary(stl_file *stl, const ADMESH_CHAR *file,
                      const char *label) {
  FILE *fp;
  int i;
  // char *error_msg;
...
void stl_write_quad_object(stl_file *stl, ADMESH_CHAR *file) {
  FILE *fp;
  int i;
  int j;
  // char *error_msg;
...
void stl_write_dxf(stl_file *stl, ADMESH_CHAR *file, char *label) {
  int i;
  FILE *fp;
  // char *error_msg;
...
0\nLAYER\n2\n0\n70\n0\n62\n7\n6\nCONTINUOUS\n0\nENDTAB\n0\nENDSEC\n");
  ...
  for (i = 0; i < stl->stats.number_of_facets; i++) {
    ...
    fprintf(fp, "10\n%f\n20\n%f\n30\n%f\n", stl->facet_start[i].vertex[0].x,
            stl->facet_start[i].vertex[0].y, stl->facet_start[i].vertex[0].z);
    fprintf(fp, "11\n%f\n21\n%f\n31\n%f\n", stl->facet_start[i].vertex[1].x,
            stl->facet_start[i].vertex[1].y, stl->facet_start[i].vertex[1].z);
    fprintf(fp, "12\n%f\n22\n%f\n32\n%f\n", stl->facet_start[i].vertex[2].x,
            stl->facet_start[i].vertex[2].y, stl->facet_start[i].vertex[2].z);
    fprintf(fp, "13\n%f\n23\n%f\n33\n%f\n", stl->facet_start[i].vertex[2].x,
            stl->facet_start[i].vertex[2].y, stl->facet_start[i].vertex[2].z);
  }
...
void stl_clear_error(stl_file *stl) { stl->error = 0; }
...
int stl_get_error(stl_file *stl) { return stl->error; }
