...
void stl_generate_shared_vertices(stl_file *stl) {
  ...
  stl->v_indices = (v_indices_struct *)calloc(stl->stats.number_of_facets,
                                              sizeof(v_indices_struct));
  if (stl->v_indices == NULL)
    perror("stl_generate_shared_vertices");
  stl->v_shared = (stl_vertex *)calloc((stl->stats.number_of_facets / 2),
                                       sizeof(stl_vertex));
  ...
  for (i = 0; i < stl->stats.number_of_facets; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (stl->stats.shared_vertices == stl->stats.shared_malloced) {
        ...
        stl->v_shared = (stl_vertex *)realloc(
            stl->v_shared, stl->stats.shared_malloced * sizeof(stl_vertex));
        if (stl->v_shared == NULL)
          perror("stl_generate_shared_vertices");
      }
...
void stl_write_off(stl_file *stl, ADMESH_CHAR *file) {
  int i;
  FILE *fp;
  // char      *error_msg;
  ...
  for (i = 0; i < stl->stats.shared_vertices; i++) {
    fprintf(fp, "\t%f %f %f\n", stl->v_shared[i].x, stl->v_shared[i].y,
            stl->v_shared[i].z);
  }
  for (i = 0; i < stl->stats.number_of_facets; i++) {
...
void stl_write_vrml(stl_file *stl, ADMESH_CHAR *file) {
  int i;
  FILE *fp;
  // char *error_msg;
