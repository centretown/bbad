...
PROJECT_BUILD_PATH    ?= .
PROJECT_SOURCE_FILES  ?= src/bbad.cpp \
	src/gamepad.cpp src/keyboard.cpp src/mouse.cpp src/state.cpp \
	src/compass.cpp src/circle.cpp

# raylib library variables
RAYLIB_SRC_PATH       ?= /home/dave/src/raylib/src
RAYLIB_INCLUDE_PATH   ?= $(RAYLIB_SRC_PATH)
RAYLIB_LIB_PATH       ?= $(RAYLIB_SRC_PATH)
DAVLIB_INCLUDE_PATH   ?=/home/dave/src/davlib
...
BUILD_WEB_HEAP_SIZE   ?= 256MB
BUILD_WEB_STACK_SIZE  ?= 2MB
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ...
    LDLIBS = web/libraylib.a
    PROJECT_BUILD_PATH = www
endif
ifeq ($(PLATFORM),PLATFORM_DRM)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ...
    ifeq ($(PLATFORM_OS),OSX)
		rm -f src/*.o external/*.o $(PROJECT_NAME)
    endif
endif
ifeq ($(PLATFORM),PLATFORM_DRM)
